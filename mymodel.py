...
class UltraNet(nn.Module):
    def __init__(self):
        ...
        print("4 bit model, 8 bit firstlast")
        W_BIT = 4
        A_BIT = 4
        conv2d_q = conv2d_Q_fn(W_BIT)
        conv2d_q_special = conv2d_Q_fn(8)
        # act_q = activation_quantize_fn(4)

        self.layers = nn.Sequential(
            conv2d_q_special(3, 16, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(16),
            ...
            conv2d_q_special(64, 36, kernel_size=1, stride=1, padding=0)
