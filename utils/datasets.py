...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if(self.istxt):
            ...
            pbar = tqdm(newlines, desc='Caching labels')
            ...
            for i, file in enumerate(pbar):
                ...
                # 读取xml中的标签
                # l = analyze_xml(file)
                l = np.array([[0, newlines[i][1][2], newlines[i][1][3], newlines[i][1][0], newlines[i][1][1]]], dtype='float32')
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        interp = cv2.INTER_LINEAR if self.augment else cv2.INTER_LINEAR  # LINEAR for training, AREA for testing
